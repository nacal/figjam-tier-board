...
Import `manifest.json` (generated by the build) through **Plugins → Development → Import plugin from manifest…** in the Figma desktop app.

Two properties were declared to Figma when the plugin was submitted, so changing either means the declaration has to be revisited:

- **It makes no network requests.** `networkAccess.allowedDomains` is `["none"]`, and no `fetch`, `XMLHttpRequest`, `WebSocket`, `sendBeacon` or `EventSource` appears in either bundle.
- **It stores only in the file and on the machine** — twelve `setPluginData` keys on nodes and one `figma.clientStorage` key for the auto-arrange toggle. Nothing is sent anywhere.
