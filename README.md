...
| 表ごと移動 | 盤面はひとつのセクション。掴んで動かせば中身ごと動く |
| 行の追加 | 末尾に新しい行を追加 |
...
- 盤面のセクションに入っていないティア行を見つけたら、包み直す。盤面を包む前のバージョンで作られた表も、これで表ごと動かせるようになる
...
- `figma.currentPage.on('nodechange')` で拾う。ドラッグ中に割り込むと掴んでいる付箋が飛ぶので 320ms デバウンスする。`origin` が `REMOTE`（他の参加者の操作）には反応しない — 全員が同じ行を奪い合って動かし続けるため
- **変更のあった行だけを並べ直す。** 全行を並べ直すと、ある行を触っただけで別の行の順位まで勝手に組み替わって見える。付箋を行の外へ出したときのために、前回の整列時点でどの付箋がどの行にいたかを覚えておき、出ていった元の行も対象に入れる
...
- 盤面の名前は各行の plugin data に持たせ、キャンバスにも見出しのテキストノードとして出す。パネルの中だけに持っていても、一緒に見ている人には見えないため
