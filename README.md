...
| リネーム | 行の名前を変更 |
| 色変更 | 行の色をプリセットから選択 |
| 並び替え | 行の順序を入れ替え（パネルの ↑↓、または ⠿ のドラッグ&ドロップ）。キャンバス上で行をドラッグしても順序は変わらない |
...
- **行の順序は plugin data（`figjamTierIndex`）に持つ。** ここだけは位置を正としない。位置を正にすると行をドラッグしただけで順序が変わり、掴んだ拍子に表が組み替わる。順序を変えるのはパネルからだけ
- 順序を持たない行（順序を持たせる前に作られた盤面、他の盤面から移してきた行）は、今の見た目の順で採番する。番号の重複も、詰め直しのたびに 0..n-1 で書き戻して解ける
- キャンバス上で行を動かしても、次の詰め直しで元のスロットへ戻る。行はロックできない（`locked` は子にも効き、中の付箋が掴めなくなる）ので、戻すことで「動かせない」を実現している。行そのものが動いたときはデバウンスを 80ms に縮めて、待たずに戻す
